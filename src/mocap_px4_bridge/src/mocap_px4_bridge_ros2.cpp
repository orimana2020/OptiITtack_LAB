...
public:
	MocapPX4Bridge() : Node("mocap_px4_bridge") {
		this->declare_parameter("mocap_topic", "/rigid_budies");
		...
		RCLCPP_INFO(get_logger(), ("mpx4_topic: " + px4_topic).c_str());

		mocap4r2_msgs::msg::RigidBody rb;
		poseSub = this->create_subscription<mocap4r2_msgs::msg::RigidBodies>(mocap_topic, 10, std::bind(&MocapPX4Bridge::poseCallback, this, _1));
		odomPub = this->create_publisher<px4_msgs::msg::VehicleOdometry>(px4_topic, 10);
	}

private:
	void poseCallback(const mocap4r2_msgs::msg::RigidBodies::UniquePtr);

	rclcpp::Subscription<mocap4r2_msgs::msg::RigidBodies>::SharedPtr poseSub;
	rclcpp::Publisher<px4_msgs::msg::VehicleOdometry>::SharedPtr odomPub;
};

void MocapPX4Bridge::poseCallback(const mocap4r2_msgs::msg::RigidBodies::UniquePtr poseMsg){
	for (const mocap4r2_msgs::msg::RigidBody& drone : poseMsg->rigidbodies){
		...
		odomMsg.position[0] = drone.pose.position.x;
		odomMsg.position[1] = -drone.pose.position.y;
		odomMsg.position[2] = -drone.pose.position.z;

		odomMsg.q[0] = drone.pose.orientation.w;
		odomMsg.q[1] = drone.pose.orientation.x;
		odomMsg.q[2] = - drone.pose.orientation.y;
		odomMsg.q[3] = - drone.pose.orientation.z;
